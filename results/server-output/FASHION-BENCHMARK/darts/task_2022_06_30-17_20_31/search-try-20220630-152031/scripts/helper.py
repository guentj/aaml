...
def getData(key, epsilon=4):
    ...
    if key == "mini":
        ...
        return x_poison, x_train, y_train, x_test,y_test
...
def createPoisonPrePoisoned(x_train, y_train, x_poisoned, poison_percent):
    y_classes = len(np.unique(y_train))
    # Can create partly poisoned Dataset from fully clean and fully poisoned Dataset by mixing
    print(f"Starting Poisoning creation with {int(poison_percent*100)}% of the Dataset")
    print(f"For the Dataset with {int(len(y_train))} Samaples this results in {int(poison_percent*len(y_train))} poisons and {int(poison_percent*len(y_train) / y_classes)} per class")
        # Get random indices for each class
    poison_idx_temp = _getPoisonedIndex(y_train, y_classes, poison_percent)
    # Get absolute values of indices to poison
    # Split values based on their class
    y_class_sorted = _getSplitBasedOnY(y_train, y_classes)
    poison_idx = []
    ...
    x_final = np.copy(x_train)
    for val in poison_idx:
        x_final[val] = x_poisoned[val]
    return np.asarray(x_final)
