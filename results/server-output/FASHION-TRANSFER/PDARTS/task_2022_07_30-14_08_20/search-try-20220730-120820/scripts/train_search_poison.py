...
from helper import getData, AutoMLPathGenerator, CustomDataset, createPoisonPrePoisoned
...
parser.add_argument('--save', type=str, default=base_path, help='experiment path')
...
def main(train_data, test_data):
    ...
    for sp in range(len(num_to_keep)):
        ...
        for epoch in range(epochs):
            ...
            criterion = nn.CrossEntropyLoss()
            criterion = criterion.cuda()
            test_queue = torch.utils.data.DataLoader(test_data, batch_size=args.batch_size, shuffle=False, pin_memory=False, num_workers=1)
            test_acc, test_obj = infer(test_queue, model, criterion)
            #INSERTED

            logging.info('Test_acc %f', test_acc)
            #model.eval()
            path = base.getModelPath()
            print("Path:", path)
...
if __name__ == '__main__':
    ...
    print(f"Epsilon = {4} and poison_percent = {0.9}")
    x_poison, x_train, y_train, x_test,y_test = getData("poison", epsilon=4)
    x_train_malicious = createPoisonPrePoisoned(x_train, y_train, x_poison, poison_percent=0.9)
    custom_train = CustomDataset(x_train_malicious, y_train)
    ...
    start_time = time.time()
    print(f"Epsilon = {4} and poison_percent = {1}")
    x_poison, x_train, y_train, x_test,y_test = getData("poison", epsilon=4)
    x_train_malicious = createPoisonPrePoisoned(x_train, y_train, x_poison, poison_percent=1)
    custom_train = CustomDataset(x_train_malicious, y_train)
    custom_test = CustomDataset(x_test, y_test)
    main(custom_train, custom_test)
    end_time = time.time()
    ...
    for epsilon in epsilons:
        for poison_percent in poison_percentages:
            start_time = time.time()
            print(f"Epsilon = {epsilon} and poison_percent = {poison_percent}")
            x_poison, x_train, y_train, x_test,y_test = getData("poison", epsilon=epsilon)
            x_train_malicious = createPoisonPrePoisoned(x_train, y_train, x_poison, poison_percent=poison_percent)
            custom_train = CustomDataset(x_train_malicious, y_train)
            custom_test = CustomDataset(x_test, y_test)
            main(custom_train, custom_test)
            end_time = time.time()
